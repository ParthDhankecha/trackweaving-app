...
class _ProductionReportPageState extends State<ProductionReportPage> {
              ...
              return Column(
                ...
                children: [
                      ...
                      Obx(
                        () => Expanded(
                          child: BuildDatefield(
                            context: context,
                            title: 'from_date'.tr,
                            selectedDate: reportController.startDate.value,
                            onDateSelected: (newDate) {
                              reportController.startDate.value = newDate;
                              reportController.endDate.value = newDate;
                            },
                          ),
                        ),
                      ),
                      ...
                      Obx(
                        () => Expanded(
                          child: BuildDatefield(
                            context: context,
                            title: 'end_date'.tr,
                            selectedDate: reportController.endDate.value,
                            onDateSelected: (newDate) {
                              reportController.endDate.value = newDate;
                            },
                          ),
                  ...
                  ShiftDropdown(
                    ...
                    selectedValue: reportController.selectedShift.value,
                    onChanged: (value) {
                    ...
                    children: [
                      ...
                      Obx(
                        () => BuildCheckboxRow(
                          ...
                          value: reportController.isGroupVisible.value,
                          onChanged: (value) {
                            reportController.changeGroupVisible();
                            reportController.filerMachineByGroup('select');
                          },
                        ),
                      ),
                    ],
                  ...
                  Visibility(
                    ...
                    child: MachineGroupDropdown(
                      ...
                      items: reportController.availableMachineGroupList,
                      onChanged: (value) {
                        reportController.selectAllMachines.value = false;
                        reportController.selectedMachineList.clear();

                        if (value?.id == 'all') {
                          reportController.filerMachineByGroup('select');
                        } else {
                          reportController.selectedMachineGroupId.value =
                              value!.id;

                          reportController.filerMachineByGroup(value.id);
                        }
                      },
                    ),
                  ),
                    ...
                    return BuildCheckboxRow(
                      ...
                      onChanged: (value) {
                        reportController.onSelectAllChanged(
                          value,
                          groupId: reportController.isGroupVisible.value
                              ? (reportController.selectedMachineGroupId.value)
                              : 'select',
                        );
                      },
                  ...
                  Obx(
                    () => Wrap(
                      ...
                      children: reportController.checkboxMachineList.map((
                        machine,
                      ) {
                        return BuildCheckboxRow(
                          title: machine.machineCode,
                          value: reportController.selectedMachineList.contains(
                            machine,
                          ),
                          onChanged: (value) {
                            reportController.onMachineSelect(machine, value);
                          },
                        );
                      }).toList(),
                    ),
  ...
  Future<void> _viewReports() async {
    try {
      var reportData = await reportController.getReportData();
      if (reportData != null) {
        Get.to(() => ReportResultScreen(reportResponse: reportData));
      }
    } catch (e) {
      Get.snackbar(
        'Error',
        'Failed to get reports data: $e',
